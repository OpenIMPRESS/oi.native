	...
	*/
    ...
    public:
        ...
        Stream(StreamID _streamID, uint32_t _streamIdx, Session * const _session,
               ...
               std::unique_ptr<DataObjectT>  (*_read)(const StreamID &streamID, std::istream * in,  std::unique_ptr<DataObjectT> data, uint64_t len),
               std::unique_ptr<DataObjectT> (*_write)(const StreamID &streamID, std::ostream * out, std::unique_ptr<DataObjectT> data, uint64_t & timestamp_out));
    ...
    private:
		...
		std::unique_ptr<DataObjectT>(*read)(const StreamID &streamID, std::istream * in,  std::unique_ptr<DataObjectT> data, uint64_t len);
		std::unique_ptr<DataObjectT>(*write)(const StreamID &streamID, std::ostream * out, std::unique_ptr<DataObjectT> data, uint64_t & timestamp_out);
        oi::core::worker::ObjectPool<DataObjectT> * const src_pool;
        oi::core::worker::WorkerQueue<DataObjectT> * const out_queue;
    };
    ...
    public:
        ...
        std::vector<oi::core::OI_META_ENTRY> * entries_at_time(uint32_t streamIdx, int64_t time);
        int64_t prev_entry_time(uint32_t streamIdx, int64_t time); // return the first smaller timestamp
        int64_t next_entry_time(uint32_t streamIdx, int64_t time); // return the first bigger timestamp
		...
		template<class DataObjectT>
		std::shared_ptr<Stream<DataObjectT>> loadStream(const StreamID &streamID,
            oi::core::worker::ObjectPool<DataObjectT> * const src_pool,
            oi::core::worker::WorkerQueue<DataObjectT> * const out_queue,
			std::unique_ptr<DataObjectT>  (*read)(const StreamID &streamID, std::istream * in,  std::unique_ptr<DataObjectT> data, uint64_t len),
			std::unique_ptr<DataObjectT> (*write)(const StreamID &streamID, std::ostream * out, std::unique_ptr<DataObjectT> data, uint64_t & timestamp_out)) {
            ...
            if (itStreams != streams.end()) {
				return std::reinterpret_pointer_cast<Stream<DataObjectT>>(itStreams->second);
			...
			if (streamIdx == metaFileHeader.streamCount) {
				metaFileHeader.streamHeaders[streamIdx].streamIdx = streamIdx;
			...
			std::shared_ptr<Stream<DataObjectT>> res = std::make_shared<Stream<DataObjectT>>(streamID, streamIdx, this, src_pool, out_queue, read, write);
			streams.insert(std::make_pair(streamID, std::reinterpret_pointer_cast<Stream<DataObject>>(res)));
			return res;
		}
		...
		void initWriter();
		void writeMetaEntry(uint32_t streamIdx, uint64_t originalTimestamp, uint64_t data_start, uint64_t data_length);
        void setStart();
        void setEnd();
    private:
		...
		std::map<StreamID, std::shared_ptr<Stream<oi::core::worker::DataObject>>> streams;
    ...
    void Stream<DataObjectT>::flush() {
        ...
        while (true) {
            ...
            uint64_t data_start = dataFile.tellp();
            {
                ...
                if (!doa.data) break;
                doa.data = std::move(this->write(streamID, &dataFile, std::move(doa.data), timestamp_out));
            }
    ...
    int64_t Stream<DataObjectT>::play(int64_t t, bool forwards, bool skip) {
        ...
        do {
            ...
            while (it != list->end()) {
				...
				if (!doa.data) {
					printf("BUFFER FULL: PLAY\n");
					break;
				}
                doa.data = std::move(this->read(streamID, &dataFile, std::move(doa.data), it->data_length));
	...
	template<class DataObjectT>
	void Stream<DataObjectT>::endStream() {
		dataFile.close();
	}
